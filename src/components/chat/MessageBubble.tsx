...
export default function MessageBubble({ item, isMe, onLongPress, onImagePress, replyContext }: MessageBubbleProps) {
  ...
  const renderContent = () => {
    if (item?.isDeletedForEveryone) {
      return (
        <Text style={[styles.bubbleText, styles.deletedText, { color: isMe ? 'rgba(255,255,255,0.7)' : themeColors.textSecondary }]}>
    ...
    switch (item.type) {
      ...
      default:
        return (
          <Text style={[styles.bubbleText, { color: isMe ? '#FFFFFF' : themeColors.text }]}>
            {item.text}
          </Text>
        );
    }
    ...
    <View style={[styles.container, isMe ? styles.myContainer : styles.friendContainer]}>
      <TouchableOpacity
        ...
        style={[
          styles.bubble,
          {
            backgroundColor: isMe ? colors.coral : isDark ? themeColors.card : '#FFFFFF',
            borderBottomRightRadius: isMe ? 4 : 22,
            borderBottomLeftRadius: isMe ? 22 : 4,
            borderColor: isMe ? 'transparent' : isDark ? themeColors.overlayLight : '#E2E8F0',
      ...
      >
        ...
        <View style={styles.meta}>
          <Text style={[styles.time, { color: isMe ? 'rgba(255,255,255,0.85)' : themeColors.textSecondary }]}>
          ...
          {isMe && (
            <View style={styles.statusContainer}>
              <Ionicons
                ...
                size={16}
                color={item.isRead || item.status === 'read' ? colors.mint : 'rgba(255,255,255,0.85)'}
